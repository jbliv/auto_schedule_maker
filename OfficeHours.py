...
class Officer:
    ...
    def __str__(self) -> str:
        return "Name: " + self.__Name + " | Reliability: " + str(self.__Reliability) + " | Order: " + str(self.__Order) + " |\n" + self.__Schedule.__str__()
...
class Schedule:
    ...
    def __str__(self) -> str: 
        ...
        hours = ["9:00 - 9:30  |", "9:30 - 10:00 |", "10:00 - 10:30|", "10:30 - 11:00|", "11:00 - 11:30|", "11:30 - 12:00|", "12:00 - 12:30|", "12:30 - 1:00 |", "1:00 - 1:30  |", "1:30 - 2:00  |", "2:00 - 2:30  |", "2:30 - 3:00  |", "3:00 - 3:30  |", "3:30 - 4:00  |", "4:00 - 4:30  |", "4:30 - 5:00  |"]
        output = "Hours:       | Monday      | Tuesday     | Wednesday   | Thursday    | Friday      |\n" + "____________________________________________________________________________________\n"
        tracer = 0
        ...
        for time in self.__Schedule:
            ...
            tracer += 1
        output = output + "Total Hrs Avail.: " + stats[1] + " | Total Hrs Unavail.: " + stats[0] + " | Pref. Hrs: " + stats[2] + " | Ideal Hrs " + stats[3] + " |"
        return output
...
class MasterMaker:
    ...
    def createSchedules(self, officers) -> list:
        ...
        minPref = 40
        currentAvailCount = 0
        stack = [(currentSchedule, currentAvail, currentHoursDict, currentUnidealSet, currentAvailCount)]
        ...
        while stack != []:
            ...
            currentSchedule, currentAvail, currentHoursDict, currentUnidealSet, currentAvailCount = stack.pop(stackCheck)
                ...
                for time in range(len(currentAvail)):
                    for day in range(len(currentAvail[time])):
                        ...
                        # Checks if slot only has 1 possible availability.
                            ...
                            if nameToObjectDict[soleOfficer].getSchedule()[time][day] == 1:
                                currentAvailCount += 1
                            ...
                            stack.append((currentSchedule, currentAvail, currentHoursDict, currentUnidealSet, currentAvailCount))
                ...
                for time in range(len(currentAvail)):
                    for day in range(len(currentAvail[time])):
                        for officer in currentAvail[time][day]:
                            ...
                            copyAvail[time][day] = []
                            if ruleFollowed:
                                stack.append((copySchedule, copyAvail, copyHoursDict, copyUnidealSet, copyAvailCount))
                        if operationDone:
                            break
                    if operationDone:
                        break
        ...
        for solution, hourcount in solutions:
            ...
            for officer in hourcount:
                queue[nameToObjectDict[officer].getOrder() - 1] = (nameToObjectDict[officer], (3 - hourcount[officer]))
            for officer, count in queue:
                if count != 0:
                    extraHours = []
                    for time in range(len(solution)):
                        for day in range(len(solution[time])):
                            if type(solution[time][day]) == str and officer.getSchedule()[time][day] != 0:
                                ...
                                if len(extraHours) < count:
                                    ...
                                    for hour in range(len(extraHours)):
                                        checkVal = officer.getSchedule()[extraHours[hour][0]][extraHours[hour][1]]
                                        if checkVal < minVal:
                                            minVal = checkVal
                                            minLoc = hour
                    if len(extraHours) == 0:
                ...
                for time in range(timeOpen):
                    ...
                    for officer in currentSlot:
                        ...
                        if nameToObjectDict[officer].getSchedule()[time][day] == 1:
                            ...
                            if time != 0 and time != 7 and (nameToObjectDict[officer].getSchedule()[time - 1][day] == 2 or nameToObjectDict[officer].getSchedule()[time - 1][day] == 3) and (nameToObjectDict[officer].getSchedule()[time + 1][day] == 2 or nameToObjectDict[officer].getSchedule()[time + 1][day] == 3):
                                middleHours += 1
                ...
                for officerOne in range(n):
                    for officerTwo in range(0, n - officerOne - 1):
                        ...
                        if data[officers[officerTwo]] > data[officers[officerTwo + 1]]:
                            officers[officerTwo], officers[officerTwo + 1] = officers[officerTwo + 1], officers[officerTwo]
                        elif data[officers[officerTwo]] == data[officers[officerTwo + 1]]:
                            if officers[officerTwo].getOrder() > officers[officerTwo + 1].getOrder():
                                officers[officerTwo], officers[officerTwo + 1] = officers[officerTwo + 1], officers[officerTwo]
        ...
        while not legalSchedules:
            ...
            if allIdeal:
                ...
                for i in self.__Officers:
                    scheduleSet.append(Officer(i.getName(), i.getScheduleObject().idealOnly().hourSegments(), i.getOrder(), i.getReliability()))
                allIdeal = False
            elif preferCycle < len(self.__Officers):
                scheduleSet = []
                for i in self.__Officers:
                    if len(self.__Officers) - preferCycle <= i.getOrder():
                        scheduleSet.append(Officer(i.getName(), i.getScheduleObject().preferredOnly().hourSegments(), i.getOrder(), i.getReliability()))
                    else:
                        scheduleSet.append(Officer(i.getName(), i.getScheduleObject().idealOnly().hourSegments(), i.getOrder(), i.getReliability()))
                preferCycle += 1
                ...
                for i in self.__Officers:
                    if len(self.__Officers) - availCycle <= i.getOrder():
                        scheduleSet.append(Officer(i.getName(), i.getScheduleObject().hourSegments(), i.getOrder(), i.getReliability()))
                    else:
                        scheduleSet.append(Officer(i.getName(), i.getScheduleObject().preferredOnly().hourSegments(), i.getOrder(), i.getReliability()))
                availCycle += 1
            else:
            ...
            if self.emptySlots(self.nameAvailability(scheduleSet)):
                ...
                availBools[-1] = True
                while availBools != [True for x in range(len(self.__Officers))] and possibles == []:
                    scheduleSet = []
                    ...
                    for i in range(len(self.__Officers)):
                        if i < availCycle:
                            if availBools[2 * len(self.__Officers) - self.__Officers[i].getOrder()]:
                                scheduleSet.append(Officer(self.__Officers[i].getName(), self.__Officers[i].getScheduleObject().hourSegments(), self.__Officers[i].getOrder(), self.__Officers[i].getReliability()))
                            elif availBools[len(self.__Officers) - self.__Officers[i].getOrder()]:
                                scheduleSet.append(Officer(self.__Officers[i].getName(), self.__Officers[i].getScheduleObject().preferredOnly().hourSegments(), self.__Officers[i].getOrder(), self.__Officers[i].getReliability()))
                            else: 
                                scheduleSet.append(Officer(self.__Officers[i].getName(), self.__Officers[i].getScheduleObject().idealOnly().hourSegments(), self.__Officers[i].getOrder(), self.__Officers[i].getReliability()))
                        elif availBools[len(self.__Officers) - self.__Officers[i].getOrder()]:
                            scheduleSet.append(Officer(self.__Officers[i].getName(), self.__Officers[i].getScheduleObject().preferredOnly().hourSegments(), self.__Officers[i].getOrder(), self.__Officers[i].getReliability()))
                        else:
                            scheduleSet.append(Officer(self.__Officers[i].getName(), self.__Officers[i].getScheduleObject().idealOnly().hourSegments(), self.__Officers[i].getOrder(), self.__Officers[i].getReliability()))
...
def main():
    ...
    officers = []
    print("You will now be asked to input the order that each officer filled out their availability")
    print("Do this by entering a number 1-16, 1 being filled out first 16 being filled out last")
    print("Please be sure to enter each number only once if you enter one twice you will be asked to fix it")
    numSet = set()
    for i in range(numOfficers):
        ...
        while not necessary:
            ...
            for _ in range(timeIntervals):
                currentTime = []
                for day in range(5):
                    color = translateColor(avail.cell(row = currentRow, column = (day + 1) * 2).fill.start_color.index)
                    if color == 6:
    ...
    finalColors = ["FFF56363", "FFFFABAB", "FFFFA97E", "FFFFE599", "FFF7FF36", "FFA9E178", "FF95B786", "FF63D297", "FFBBEAE7", "FF74A0E9", "FFA4C2F4", "FF8AA1C8", "FFB4A6FF", "FFD9D2E9", "FFD5A6BD", "FFDF61A3", "FFEF135B"]
    nameToColorDict = {officers[i].getName() : finalColors[i] for i in range(len(officers))}
    ...
    for schedule in finalSchedules:
        ...
        for time in range(len(schedule)):
            for day in range(len(schedule[time])):
                if type(schedule[time][day]) != tuple:
                    cell = currentSheet.cell(row = (startRow + 2 * time), column = (startColumn + 2 * day))
                    cell.value = schedule[time][day]
                    cell.fill = PatternFill("solid", fgColor = nameToColorDict[schedule[time][day]])
                else:
                    currentSheet.unmerge_cells(start_row = (startRow + 2 * time), start_column = (startColumn + 2 * day), end_row = (1 + startRow + 2 * time), end_column = (1 + startColumn + 2 * day))
                    currentSheet.merge_cells(start_row = (startRow + 2 * time), start_column = (startColumn + 2 * day), end_row = (1 + startRow + 2 * time), end_column = (startColumn + 2 * day))
                    cellOne = currentSheet.cell(row = (startRow + 2 * time), column = (startColumn + 2 * day))
                    cellOne.value = schedule[time][day][0]
                    cellOne.fill = PatternFill("solid", fgColor = nameToColorDict[schedule[time][day][0]])
                    currentSheet.merge_cells(start_row = (startRow + 2 * time), start_column = (1 + startColumn + 2 * day), end_row = (1 + startRow + 2 * time), end_column = (1 +startColumn + 2 * day))
                    cellTwo = currentSheet.cell(row = (startRow + 2 * time), column = (1 + startColumn + 2 * day))
                    cellTwo._style = copy.copy(cellOne._style)
                    cellTwo.value = schedule[time][day][1]
                    cellTwo.fill = PatternFill("solid", fgColor = nameToColorDict[schedule[time][day][1]])
